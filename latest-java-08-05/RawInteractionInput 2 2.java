...
-- [1] Resolve Linked Leads
LET $linked_leads = IF $contact_id != NONE THEN 
    (SELECT VALUE lead_ids FROM customer_contact WHERE $contact_id INSIDE contact_ids LIMIT 1)[0] 
    ELSE [$lead_rec] END;
LET $linked_leads = IF $linked_leads != NONE AND array::len($linked_leads) > 0 
    THEN array::distinct(array::concat($linked_leads, [$lead_rec])) 
    ELSE [$lead_rec] END;

-- [2] Fetch aggregated conversations
LET $convs = SELECT *, type::string(channel) AS channel_str 
             FROM conversation WHERE lead IN $linked_leads ORDER BY created_at ASC;

-- [3] Message Counts (New Logic Kept)
LET $wa_conv_ids = $convs[?string::contains(channel_str, "whatsapp")].id;
LET $ph_conv_ids = $convs[?string::contains(channel_str, "phone") OR string::contains(channel_str, "voice")].id;
LET $em_conv_ids = $convs[?string::contains(channel_str, "email")].id;

LET $wa_count = (SELECT count() FROM message WHERE conversation IN $wa_conv_ids GROUP ALL)[0].count OR 0;
LET $ph_count = (SELECT count() FROM message WHERE conversation IN $ph_conv_ids GROUP ALL)[0].count OR 0;
LET $em_count = (SELECT count() FROM message WHERE conversation IN $em_conv_ids GROUP ALL)[0].count OR 0;
LET $total    = $wa_count + $ph_count + $em_count;

-- [4] Sentiment Aggregation (RESTORED)
LET $sent_map = array::map($convs, |$c|
    ...
    ELSE IF $c.sentiment = "DISSPOINTED"                       THEN -0.5
...
LET $dom_ch = IF $wa_count >= $ph_count AND $wa_count >= $em_count THEN "whatsapp"
              ...
              ELSE "email" END;
LET $first_ch  = ($convs[0].channel_str ?? "email");
LET $last_conv = ($convs[array::len($convs) - 1] ?? {});
LET $last_ch   = ($last_conv.channel_str ?? "email");

-- [7] Duration Logic (New Logic Kept)
LET $first_ts = ($convs[0].created_at ?? $now);
LET $all_last_ts = array::sort($convs.last_message_at);
LET $raw_last_ts = array::last($all_last_ts) ?? $last_conv.created_at ?? $now;
LET $last_ts = IF $raw_last_ts < $first_ts THEN $first_ts ELSE $raw_last_ts END;

LET $time_hours_raw = IF array::len($convs) > 0
...
-- [9] Ordinals
LET $lead_state = type::string($lead.state);
LET $stage_ord = IF $lead_state = "NEW_LEAD"              THEN 0
    ELSE IF $lead_state = "MANUAL_FOLLOW_UP"              THEN 1
    ELSE IF $lead_state = "SALES_FOLLOW_UP"               THEN 2
    ELSE IF $lead_state = "SALES_QUALIFIED"               THEN 3
    ELSE IF $lead_state = "QUOTATION_DRAFT" OR $lead_state = "QUOTATION" THEN 4
    ELSE IF $lead_state = "FOLLOW_UP_NEGOTIATION"         THEN 5
    ELSE IF $lead_state = "CLOSED"                        THEN 6
    ELSE 0 END;

LET $dom_ord = IF string::contains($dom_ch, "email")    THEN 0
               ELSE IF string::contains($dom_ch, "phone") THEN 1
               ELSE 2 END;
...
SELECT
    ...
    $stage_ord                               AS stage_ordinal,
    ($dom_ord * 6 + $stage_ord)              AS channel_x_stage,
    (organisation.historical_deals_won ?? 0) AS historical_deals_won
FROM $lead_rec;
    ...
    public double callMlPredict(NormalizedInteractionInput norm) {
        ...
        String sql = String.format(Locale.US, """
            LET $prediction = ml::interaction_score<2.0.0>([
