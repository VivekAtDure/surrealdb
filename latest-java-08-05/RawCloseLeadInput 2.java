...
-- Find all leads linked to this contact
-- FIX: Added fallback ?? [] to handle cases where contact_id is missing or query returns NONE
...
-- FIX: Ensure array::concat receives two arrays, even if $linked_leads_raw is NONE
LET $all_leads = array::distinct(array::concat(($linked_leads_raw ?? []), [$lead_rec]));

-- FIX: Ensure array::map receives an array
LET $all_lids  = array::map(($all_leads ?? []), |$r| type::string($r));

-- [2] Aggregate Data Across Cluster
-- FIX: Fallback to empty array to prevent issues in the projection block
LET $conv_res = (
...
-- [2b] Latest message
LET $latest_msg_res = (
    SELECT content, has_urgency_keyword, has_price_mention, created_at
    FROM message
    WHERE conversation IN (SELECT VALUE id FROM conversation WHERE lead IN $all_leads)
    ORDER BY created_at DESC LIMIT 1
);
LET $latest_msg = $latest_msg_res[0] ?? {};

-- [3] Fetch linked product
-- FIX: Use ?? {} for the projection block's null-coalescing logic
LET $prod_res = (SELECT VALUE out FROM interested_in WHERE in IN $all_leads LIMIT 1);
LET $prod = $prod_res[0] ?? {};

-- [4] Metrics & Org Context
LET $org = $lead_rec.organisation;
-- FIX: If no quotations exist, ensure count is 0 not NONE
LET $quotation_count = (SELECT count() FROM generated_quotation WHERE in IN $all_leads GROUP ALL)[0].count ?? 0;
LET $now = time::now();
...
SELECT
    ...
    -- FIX: added fallback for created_at to avoid math errors with NONE
    math::floor(duration::secs($now - ($conv.last_message_at ?? $conv.created_at ?? created_at ?? $now)) / 86400) AS days_since_last_msg,
    ...
    -- FIX: Wrapped in array index check to prevent sale_closed from failing if no status history exists
(IF (
    -- Select both the value and the sort field
    SELECT lead_status, created_at 
    FROM lead_status 
    WHERE leadid IN $all_lids 
    ORDER BY created_at DESC 
    LIMIT 1
-- Access the index [0] and then the specific field .lead_status
)[0].lead_status = 'CLOSED' THEN 1 ELSE 0 END) AS sale_closed
...
FROM $lead_rec;
    ...
    public double callMlPredict(NormalizedCloseLeadInput norm) {
        ...
        String sql = String.format("""
            ...
            LET $prediction = ml::close_prob<2.0.0>([
                %f,  -- [0]  n_capacity_tons
                %f,  -- [1]  n_stage_ordinal
