...
-- Ensure current lead is included and list is unique (Record IDs)
LET $linked_leads = array::distinct(array::concat($linked_leads, [$lead_rec]));
-- String IDs for log table lookups
LET $linked_lids = array::map($linked_leads, |$r| type::string($r));

-- [3] Aggregate conversation metrics across all channels
-- FIX: use $linked_lids (strings) not $linked_leads (record IDs) — conversation.lead
-- is stored as a string ID; type mismatch with record IDs returned 0 rows silently
...
-- Stage ordinal for velocity
-- FIX: added FOLLOW_UP, QUOTATION_IN_DRAFT, QUOTATION_SENT, MANUAL_FOLLOW_UP
...
-- FIX: use $linked_lids for string-based comparison (same type mismatch fix as convs above)
LET $prod = (SELECT VALUE out FROM interested_in WHERE type::string(in) IN $linked_lids LIMIT 1)[0];
-- FIX: "OR 0" is not valid SurrealQL null coalescing — use ?? 0
...
SELECT
    ...
    -- Milestone check across all linked leads (String lookup)
    (IF array::len(
    ...
    -- Corrected Master Price List Mapping
    ...
    -- Carry forward existing sub-scores
...
FROM $lead_rec;
    ...
    public double callMlPredict(NormalizedLeadQualInput norm) {
        ...
        String sql = String.format(Locale.US, """
            ...
            LET $prediction = ml::lead_qualification<1.0.0>([
